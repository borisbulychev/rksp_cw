...
// Функции для выполнения при клике на карточки
  ...
  $(".card").click(function () {
    ...
    if (cardsOpen.length <= 1) {
      ...
      $(this).replaceWith(
        `<div card open${
          cardsOpen.length
        } class="flipped" id="${CardId}" style="background-color:${
          deckId[CardId - 1].color
        }"><img src="${
          deckId[CardId - 1].image
      ...
      if (cardsOpen[0] == cardsOpen[1]) {
        ...
        $("[open1]")
          .delay(1000)
          .replaceWith(
            `<div card class="card" id="${cardsOpen2[0]}"><img src="src/question-mark.png" alt="card" width="100px" height="100px"></div>`
          );
        $("[open2]")
          .delay(1000)
          .replaceWith(
            `<div card class="card" id="${cardsOpen2[1]}"><img src="src/question-mark.png" alt="card" width="100px" height="100px"></div>`
          );
        cardsOpen = cardsNone;
